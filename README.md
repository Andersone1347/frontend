...
Работаем в консоле.
Создаём переменную **let str = "Привет";**

После ввода в консоль переменной **str.** у нас появляеться подсказда, различные свойства, способы манипуляции с переменной.
```
str.length
6
```
...
(4) [5, 2, 'Text', true]
```
В данный момент в нашем массиве 3 индекса , что бы добавить ещё один мы просто создаём 4-ый.
```
arr[4] = 7
```
Проверяем массив:
...
2: (3) [1, 0, 3]
```
И для того что бы получить нужное значение , например **3** из масива **103**.прописываем путь по индексам:
```
matrix[2][2]
3
```
...
**arr.shift()** - брать данные из начала очереди массива.
```
arr
(4) [5, 2, 'Str', true]
arr.shift()
...
(5) [5, 2, 'Str', true, 'text']
```
**arr.pop()** - забирает последнее значение.
```
arr.pop()
true
arr
...
[[Prototype]]: Object
```
Задание создать из массива и объектов логическое дерево например из питомцев.
```
let cats = [
    {
        "name": "Barsik",
